...
- **Removal is implemented but not exposed.** `removeVideo()` is in the data layer with an authoritative payload lifted from YouTube's own client, but it was never round-tripped, so the UI treats "already in" as terminal rather than as a toggle. Shipping an untested write path against a user's real playlists is not a trade worth making for one interaction.
