# Architecture proposal — YouTube Playlist Search, rebuild

Status: proposal. No implementation plan here. Decisions to make are at the end.
...
Remaining unknowns:
2. **Does the compact save picker (sparse accounts) also fire `get_add_to_playlist`?** Load-bearing for intent coverage on new users.
3. **Does a synthetic `Escape` keydown actually dismiss YouTube's dialog?** Load-bearing for the zero-selector dismissal. Fallback is one shape-gated CSS rule that fails open harmlessly.
4. **Does `webRequest.onBeforeRequest` reliably deliver the POST body for these calls, early enough?** Load-bearing for the whole intent layer. If not, MAIN-world `fetch` patching is the fallback — at the cost of tamper surface and review scrutiny.

---

## 8. Decisions I need from you

1. **Replace YouTube's dialog outright, or render ours alongside theirs?** I recommend replace — partial ownership of a surface is what got us here.
2. **When InnerTube breaks, fail closed or attempt a DOM fallback?** I strongly recommend fail closed with an honest message. A DOM fallback reintroduces the exact code path that produced every symptom in §2.
3. **Build the contract registry as a real extractable library now, or inline it and extract once Amazon is real?** Inline-but-cleanly-seamed is probably right; premature libraries are their own failure mode.
4. Confirm `/feed/playlists` stays out of scope for v1.
